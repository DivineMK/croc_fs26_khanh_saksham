...
set_input_delay -min -clock $soc_clk [expr { $SOC_TCK * 0.10 }] [ \
    get_ports {gpio_i* fan_sw* testmode_i}]
...
set_max_delay [expr { 2 * $SOC_TCK }] -from [get_ports {gpio_i* fan_sw* testmode_i}]
set_false_path -hold -from [get_ports {gpio_i* fan_sw* testmode_i}]
